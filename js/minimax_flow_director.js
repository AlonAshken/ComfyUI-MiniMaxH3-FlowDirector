...
const ANALYZE_KEY_SETTING = "MiniMaxH3Flow.Analyze.ApiKey";
...
class TimelineEditor {
  ...
  async _getOrExtractAudio(seg) {
    ...
    const extractionPromise = (async () => {
      const resp = await api.fetchApi(`/minimax_flow_director_get_audio?filename=${encodeURIComponent(fileKey)}`);
      if (resp.status === 200) {
  ...
  // own /minimax_flow_director_upload_chunk endpoint to bypass the 413 size limit.
  async _uploadVideoFile(file) {
    ...
    try {
      const checkResp = await api.fetchApi(`/minimax_flow_director_check_file?filename=${encodeURIComponent(safeFileName)}&size=${file.size}`);
    ...
    if (file.size > CHUNK_SIZE) {
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        const resp = await api.fetchApi("/minimax_flow_director_upload_chunk", { method: "POST", body: formData });
          ...
          vid.onloadeddata = async () => {
            ...
            vid.onseeked = () => {
              ...
              this._uploadVideoFile(file).then(filePath => {
                ...
                if (filePath) {
                  api.fetchApi(`/minimax_flow_director_get_audio?filename=${encodeURIComponent(filePath)}`)
                    .then(r => r.json())
                    .then(res => {
  ...
  async _placeMotionViaServer(file, frameRate, targetFrameStart) {
    try {
      ...
      const resp = await api.fetchApi("/minimax_flow_director/probe_video", {
  ...
  async runGemmaAnalysis(idx, btn) {
    ...
    try {
      const resp = await api.fetchApi("/minimax_flow_director/analyze_character", {
        method: "POST",
        body: JSON.stringify({
          clip_name: clip_name,
  ...
  showSettingsMenu(anchorEl) {
    ...
    btnOpenFolder.addEventListener("click", async () => {
      try {
        const response = await api.fetchApi("/minimax_flow_director_open_folder");
...
app.registerExtension({
  ...
  async setup() {
    ...
    app.queuePrompt = async function (...args) {
      try {
        ...
        const director = nodes.find(n => n && (n.comfyClass === "MiniMaxH3FlowDirectorCS" || n.type === "MiniMaxH3FlowDirectorCS"));
        if (director) {
          ...
          if (provider !== "off") {
            try {
              await api.fetchApi("/minimax_flow_director/unload_ollama", {
                method: "POST",
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData.name === "MiniMaxH3FlowDirectorCS") {
      ...
      nodeType.prototype.onNodeCreated = function () {
        ...
        const _mmxBuildSettingsPanel = (node, panelRoot) => {
          ...
          mpIn.type = "number"; mpIn.step = "any"; mpIn.min = "0.01"; mpIn.max = "20"; sIn(mpIn, "86px");
          mpIn.title = "Pixel budget in megapixels (e.g. 1.03, 0.75, 1.5). H3's native canvas is 1.03 MP "
          ...
          mpIn.addEventListener("focus", () => { isMpTyping = true; });
          mpIn.addEventListener("blur", () => {
            isMpTyping = false;
            applyAspectMp();
          });
          mpIn.addEventListener("keydown", (e) => {
            e.stopPropagation();
            if (e.key === "Enter") {
              mpIn.blur();
            }
        ...
        const refreshPrompt = async () => {
          try {
            ...
            const resp = await api.fetchApi("/minimax_flow_director/compile_prompt", {
